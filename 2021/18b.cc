...
node* deep_copy_vals(node *n)
{
	...
	node *ret = new node();
	ret->val = n->val;
	ret->left = deep_copy_vals(n->left);
	ret->right = deep_copy_vals(n->right);
	return ret;
}

void fixup_parents(node *n, node *parent)
{
	if (n == nullptr)
		return;

	n->parent = parent;
	fixup_parents(n->left, n);
	fixup_parents(n->right, n);
}

node* deep_copy(node *n)
{
	node *ret = deep_copy_vals(n);
	fixup_parents(ret, nullptr);
	return ret;
}

int main()
{
	// Generate pair-wise combinations.

	std::vector<node*> snails = parse_input();
	size_t n = snails.size();

	size_t size = 2 * n * (n - 1);
	std::vector<node*> combins(size);

	int k = 0;
	for (size_t i = 0; i < n - 1; i++) {
		for (size_t j = i + 1; j < n; j++) {
			combins[k++] = deep_copy(snails[i]);
			combins[k++] = deep_copy(snails[j]);
			combins[k++] = deep_copy(snails[j]);
			combins[k++] = deep_copy(snails[i]);
		}
	}
	...
	for (int i = 0; i < size; i += 2) {
		node *first = combins[i];
		node *second = combins[i + 1];

		node *added = add(first, second);
		reduce(added);

		int64_t mag = get_magnitude(added);


		max = std::max(max, get_magnitude(added));
	}
