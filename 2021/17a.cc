...
#define TRY_STEPS (500)
#define VELOCITY_X_FROM (0)
#define VELOCITY_X_TO (50)

#define VELOCITY_Y_FROM (-200)
#define VELOCITY_Y_TO (200)
...
std::pair<int, bool> try_velocity(const target& t, int velocity_x, int velocity_y)
{
	int x = 0, y = 0;
	int max_y = 0;
...
int get_max_y(const target& t)
{
	...
	for (int velocity_x = VELOCITY_X_FROM; velocity_x <= VELOCITY_X_TO; velocity_x++) {
		for (int velocity_y = VELOCITY_Y_FROM; velocity_y <= VELOCITY_Y_TO; velocity_y++) {
			auto [ new_max_y, success ] = try_velocity(t, velocity_x, velocity_y);
