...
int main()
{
	...
	while (true) {
		...
		if (line.from.y == line.to.y) {
			...
			int sign_x = (line.to.x - line.from.x) / std::abs(line.to.x - line.from.x);
			int sign_y = (line.to.y - line.from.y) / std::abs(line.to.y - line.from.y);

			int x = line.from.x;
			int y = line.from.y;

			while (x != line.to.x + sign_x && y != line.to.y + sign_y) {
				if (++grid[x][y] == 2)
					count++;
